...
$motivo = $_POST['motivo'] ?? ($estado === 'ausente' ? 'Cambio a estado ausente' : 'Reactivación a activo');
...
try {
    ...
    if ($estado === 'ausente' || $estado === 'inactivo') {
        $stmt = $conn->prepare("
            UPDATE profesor 
            SET 
    ...
    echo json_encode([
        ...
        'nuevo_estado' => $estado,
        'motivo' => ($estado === 'ausente' || $estado === 'inactivo') ? $motivo : null
    ]);
